...
while break_long_minutes < 1 or break_long_minutes > 30:
    print(Fore.RED + Back.BLACK + 'Minutes must be between 1 & 30, (Default: 20):')
    break_long_minutes = int(input() or "20")
...
while True:
    if overall_runs != 0:
        print(Fore.CYAN + Back.BLACK + Style.BRIGHT + 'Run again? (y/n):')
        answer = str(input())
    ...
    while timer_count <= iterations:
        ...
        if timer_count <= 3:
            ...
            for i in trange((break_long_minutes * 60), bar_format="{l_bar}%s{bar}%s{r_bar}" % (Fore.YELLOW, Fore.RESET)):
                time.sleep(1)
